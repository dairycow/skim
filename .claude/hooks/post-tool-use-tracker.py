...
def detect_repo(file_path: str, project_root: str) -> str | None:
    ...
    if (
        repo in python_src_patterns
        or repo in test_patterns
        or repo in docs_patterns
        or repo in script_patterns
    ):
        return repo
...
def get_lint_command(repo: str, project_root: str) -> str | None:
    """Get linting command for Python repo."""
    repo_path = (
        Path(project_root) / repo if repo != "root" else Path(project_root)
    )
    pyproject_toml = (
        repo_path / "pyproject.toml"
        if repo != "root"
        else Path(project_root) / "pyproject.toml"
    )
...
def get_format_command(repo: str, project_root: str) -> str | None:
...
def get_test_command(repo: str, project_root: str) -> str | None:
...
def main():
    try:
        ...
        if affected_repos_file.exists():
            existing_repos = set(
                affected_repos_file.read_text().strip().split("\n")
            )
