...
def crontab_content():
    path = Path(__file__).parent.parent.parent.parent / "crontab"
...
def test_crontab_runs_bot_methods(crontab_content):
    for command in ("scan", "track_ranges", "alert", "trade", "manage"):
        assert f"-m skim.trading.core.bot {command}" in crontab_content
...
def test_crontab_includes_scan_at_market_open(crontab_content):
    ...
    assert (
        "1 23 * * 0-4 skim cd /opt/skim && /opt/skim/.venv/bin/python -m skim.trading.core.bot scan"
        in crontab_content
    )
