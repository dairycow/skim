...
def validate_oauth_environment():
    ...
    if missing_vars:
        pytest.skip(
            f"Missing required environment variables: {missing_vars}. This is a manual test requiring real IBKR OAuth credentials."
        )

    try:
        from skim.trading.core.config import get_oauth_key_paths
        ...
        pytest.skip(f"OAuth key validation failed: {e}")
...
def create_gap_scan_result(gap_stocks=None, new_candidates=None):
    ...
    from skim.trading.validation.scanners import GapScanResult
...
def create_monitoring_result(gap_stocks=None, triggered_candidates=None):
    ...
    from skim.trading.validation.scanners import MonitoringResult
...
def create_or_tracking_result(gap_stocks=None, or_tracking_candidates=None):
    ...
    from skim.trading.validation.scanners import ORTrackingResult
