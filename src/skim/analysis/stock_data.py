...
class StockData:
    ...
    def load_from_csv(self, filepath: str) -> None:
        ...
        """
        df = pl.read_csv(filepath, has_header=False, new_columns=[
    ...
    def calculate_return(self, start_date: datetime, end_date: datetime) -> float | None:
    ...
    def calculate_returns_over_period(self, start_date: datetime, end_date: datetime) -> dict:
        ...
        total_return = ((end_price - start_price) / start_price) * 100 if start_price > 0 else None

        try:
            avg_volume = sum(v for v in volumes if v is not None) / len([v for v in volumes if v is not None]) if volumes else 0.0
        ...
        if len(close_prices) > 1:
            ...
            for i in range(1, len(close_prices)):
                if close_prices[i] is not None and close_prices[i-1] is not None and close_prices[i-1] != 0:
                    pct_changes.append(((close_prices[i] - close_prices[i-1]) / close_prices[i-1]) * 100)
    ...
    def filter_by_criteria(self, min_price: float = 0.20, min_volume: int = 50000) -> bool:
        """
        ...
        if len(self.df) >= 50:
            ...
            avg_volume = float(volume_mean) if volume_mean is not None and isinstance(volume_mean, (int, float)) else 0.0
        else:
