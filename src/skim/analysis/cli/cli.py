...
class CLI:
    ...
    def show_welcome(self):
        ...
        """
        self.console.print(Panel(welcome_text, title="[bold]Welcome[/bold]", border_style="cyan"))
    ...
    def load_data(self):
        ...
        self.console.print(f"[green]✓ Loaded {len(self.loader.stocks)} stocks[/green]")

    def show_top_performers(self, period: str):
        """Show top performers for a period."""
        if not self.data_loaded:
            self.console.print("[red]Please load data first using 'load' command[/red]")
            return
        ...
        self.console.print(f"[cyan]Finding top performers for {period} ({start_date.date()} to {end_date.date()})...[/cyan]")
    ...
    def show_gaps(self, period: str):
        ...
        self.console.print(f"[cyan]Scanning for gaps in {period} ({start_date.date()} to {end_date.date()})...[/cyan]")
    ...
    def show_announcements(self, ticker: str, period: str):
        ...
        self.console.print(f"[cyan]Fetching announcements for {ticker} in {period} ({start_date.date()} to {end_date.date()})...[/cyan]")

        announcements = self.scraper.get_announcements(ticker, start_date, end_date)
    ...
    def show_momentum_bursts(self, period: str, min_days: int = 3):
        ...
        self.console.print(f"[cyan]Finding momentum bursts for {period} ({start_date.date()} to {end_date.date()})...[/cyan]")
    ...
    def show_consolidations(self, period: str, max_range: float = 10.0, min_days: int = 5):
        ...
        self.console.print(f"[cyan]Finding consolidations for {period} ({start_date.date()} to {end_date.date()})...[/cyan]")
        ...
        self.momentum_scanner.display_consolidations(consolidations, self.console)
    ...
    def show_pattern_analysis(self, ticker: str, period: str):
        ...
        self.console.print(f"[cyan]Analyzing patterns for {ticker} in {period} ({start_date.date()} to {end_date.date()})...[/cyan]")

        patterns = self.momentum_scanner.analyze_stock_patterns(ticker, start_date, end_date)

        self.console.print(f"[bold]Pattern Analysis for {ticker} ({period})[/bold]\n")
        self.console.print(f"[cyan]Momentum Bursts:[/cyan] {patterns.get('total_momentum_bursts', 0)}")
        if patterns.get('momentum_bursts'):
            self.momentum_scanner.display_momentum_bursts(patterns['momentum_bursts'], self.console)
        self.console.print(f"\n[cyan]Consolidations:[/cyan] {patterns.get('total_consolidations', 0)}")
        if patterns.get('consolidations'):
            self.momentum_scanner.display_consolidations(patterns['consolidations'], self.console)
    ...
    def show_performance(self, ticker: str, period: str):
        ...
        if ticker.upper() not in self.loader.stocks:
            self.console.print(f"[red]Stock {ticker.upper()} not found in loaded data[/red]")
            return
        ...
        self.console.print(f"[cyan]Calculating performance for {ticker.upper()} in {period} ({start_date.date()} to {end_date.date()})...[/cyan]")

        stock = self.loader.stocks[ticker.upper()]
        metrics = stock.calculate_returns_over_period(start_date, end_date)

        if not metrics:
            self.console.print(f"[yellow]No data available for {ticker.upper()} in the specified period[/yellow]")
            return
        ...
        table.add_row("Return %", f"{metrics['total_return']:.2f}%" if metrics['total_return'] is not None else "N/A")
        table.add_row("Start Price", f"${metrics['start_price']:.3f}" if metrics['start_price'] else "N/A")
        table.add_row("End Price", f"${metrics['end_price']:.3f}" if metrics['end_price'] else "N/A")
        table.add_row("Avg Volume", f"{metrics['avg_volume']:,.0f}" if metrics['avg_volume'] else "N/A")
        table.add_row("Volatility %", f"{metrics['volatility']:.2f}%" if metrics['volatility'] else "N/A")
        table.add_row("Days", f"{metrics['days']}" if metrics.get('days') else "N/A")
    ...
    def show_move_statistics(self, period: str):
        ...
        self.console.print(f"[cyan]Calculating move statistics for {period} ({start_date.date()} to {end_date.date()})...[/cyan]")
    ...
    def show_company_info(self, ticker: str):
        ...
        self.console.print(f"[cyan]Fetching company info for {ticker}...[/cyan]")
        ...
        try:
            ...
            if not info or 'longName' not in info:
                self.console.print(f"[red]Ticker {ticker} not found or no data available[/red]")
                return
            ...
            table.add_row("Business Summary", first_period[:200] + "..." if len(first_period) > 200 else first_period)

            self.console.print(table)

        except Exception as e:
            self.console.print(f"[red]Error fetching info for {ticker}: {e}[/red]")
    ...
    def show_help(self):
        ...
        [cyan]quit[/cyan]
            Exit program.
        """
        self.console.print(Panel(help_text, title="[bold]Help[/bold]", border_style="yellow"))
            ...
            while True:
                try:
                    ...
                    if cmd == 'quit' or cmd == 'exit' or cmd == 'q':
                        ...
                        if len(parts) < 2:
                            self.console.print("[red]Usage: gaps <period>[/red]")
                            continue
                        self.show_gaps(' '.join(parts[1:]))

                    elif cmd == 'ann':
                        if len(parts) < 3:
                            self.console.print("[red]Usage: ann <ticker> <period>[/red]")
                            continue
                        ticker = parts[1]
                        period = ' '.join(parts[2:])
                        ...
                        if len(parts) < 2:
                            self.console.print("[red]Usage: chart <ticker> [period][/red]")
                            continue
                        ticker = parts[1]
                        ...
                        if len(parts) < 2:
                            self.console.print("[red]Usage: momentum <period>[/red]")
                            continue
                        period = ' '.join(parts[1:])
                        ...
                        if len(parts) < 2:
                            self.console.print("[red]Usage: consolidate <period>[/red]")
                            continue
                        period = ' '.join(parts[1:])
                        ...
                        if len(parts) < 3:
                            self.console.print("[red]Usage: pattern <ticker> <period>[/red]")
                            continue
                        ticker = parts[1]
                        period = ' '.join(parts[2:])
                        ...
                        if len(parts) < 3:
                            self.console.print("[red]Usage: perf <ticker> <period>[/red]")
                            continue
                        ticker = parts[1]
                        period = ' '.join(parts[2:])
                        ...
                        if len(parts) < 2:
                            self.console.print("[red]Usage: movestats <period>[/red]")
                            continue
                        period = ' '.join(parts[1:])
                        ...
                        if len(parts) < 2:
                            self.console.print("[red]Usage: info <ticker>[/red]")
                            continue
                        ticker = parts[1]
